...
const activeUserNames = users.flatMap((u) => (u.isActive ? [u.name] : []));
...
const byStatus = orders.reduce<Record<string, Order[]>>((groups, order) => {
...
@Component({
  ...
  template: `
    @for (item of items; track item.id) {
      <div class="item" [style.contain-intrinsic-size]="'0 ' + estimateHeight(item) + 'px'">
        {{ item.content }}
      </div>
    }
  `,
...
export class ProductService {
  ...
  getProductsByIds(ids: string[]): Product[] {
    return ids.map((id) => this.productsMap.get(id)).filter((p): p is Product => p !== undefined);
  }
}
```
...
// .vscode/settings.json
{
  ...
  "typescript.preferences.autoImportFileExcludePatterns": ["**/index.ts", "**/index"]
}
```
...
source$.pipe(
  switchMap((value) =>
    value.needsExtra ? fetchExtra(value).pipe(map((extra) => ({ ...value, extra }))) : of(value)
  )
);
```
...
export class UserProfileComponent {
  user$ = this.http.get<User>('/api/user').pipe(shareReplay({ bufferSize: 1, refCount: true }));
}
```
...
searchControl.valueChanges
  .pipe(mergeMap((query) => this.searchService.search(query)))
  .subscribe((results) => {
...
searchControl.valueChanges
  .pipe(concatMap((query) => this.searchService.search(query)))
  .subscribe((results) => {
...
submitForm$.pipe(switchMap(() => this.orderService.placeOrder(this.form.value))).subscribe();
// First order never placed if user clicks twice quickly

// ❌ mergeMap - Both submits go through, duplicate orders!
submitForm$.pipe(mergeMap(() => this.orderService.placeOrder(this.form.value))).subscribe();
...
itemsToSave$.pipe(switchMap((item) => this.saveItem(item))).subscribe();
// Only last item gets saved

// ❌ mergeMap - Order not guaranteed, race conditions
itemsToSave$.pipe(mergeMap((item) => this.saveItem(item))).subscribe();
...
itemsToSave$.pipe(concatMap((item) => this.saveItem(item))).subscribe({
...
export class SearchComponent {
  ...
  results$ = this.searchControl.valueChanges.pipe(
    ...
    switchMap(
      (
        query // Cancel previous request
      ) => this.searchService.search(query).pipe(catchError(() => of([])))
    )
  );
}
```
...
export class UserCardComponent {
  ...
  age = computed(() => new Date().getFullYear() - this.user().birthDate.getFullYear());
}
```
...
@Component({
  ...
  template: `
    ...
    <img [ngSrc]="user().avatar" alt="User avatar" width="64" height="64" />
  `,
})
...
@Component({
  ...
  template: `
    ...
    <cdk-virtual-scroll-viewport itemSize="80" class="product-list">
      <app-product-card
...
export class DataProcessorComponent {
  ...
  private worker = new Worker(new URL('./data-processor.worker', import.meta.url));
